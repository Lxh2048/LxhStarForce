...
namespace HybridCLR.Builder
{
    public class HybridCLRBuilderController 
    {
        public void CompileHotfixDll(int platformIndex)
        {
            ...
            string aotAssembliesDstDir = $"{Application.dataPath}/GameRes/HotAssemblies/AOT";
            foreach (var dll in SettingsUtil.AOTAssemblyNames)
            {
                ...
                Debug.Log($"[CopyAOTAssembliesTo GameRes/HotAssemblies/AOT] copy AOT dll {srcDllPath} -> {dllBytesPath}");
            }
            ...
            string hotfixAssembliesDstDir = $"{Application.dataPath}/GameRes/HotAssemblies/HotDll";
            foreach (var dll in SettingsUtil.HotUpdateAssemblyFilesExcludePreserved)
            {
                string dllPath = $"{hotfixDllSrcDir}/{dll}";
                string dllBytesPath = $"{hotfixAssembliesDstDir}/{dll}.bytes";
                File.Copy(dllPath, dllBytesPath, true);
                Debug.Log($"[CopyHotUpdateAssembliesTo GameRes/HotAssemblies/HotDll] copy hotfix dll {dllPath} -> {dllBytesPath}");
            }
